        ...
        while True: 
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    # torch.where(torch.isnan(audio_lm), torch.full_like(audio_lm, 0), audio_lm)
                    # torch.where(torch.isnan(text_lm), torch.full_like(text_lm, 0), text_lm)
                    ...
                    g_loss = 0.2 * (self.args.alpha * (advl1) + (1-self.args.alpha) * (rl1)) 
